...
// Calculating the relative length of the calendar event
  ...
  a.button_spacing = 355 * (float)first_time_as_seconds / SECONDS_IN_DAY

}

// Function checks if the user has input a valid date date, takes into account leap years
...
bool time_conflict(Event checking, vector<Event> calendar) {
  ...
  for(int i = 0; i < calendar.size() - 1; i++) {
    if(checking.date == calendar.at(i).date ) { // and calendar.at(i).hidden == true
...
// Function adds dashes, used when storing the event description in external
// file for easier file input
...
// Function removes the dashes from the event description input on open,
// stores them for use by the event class
...
int main() {
  ...
  string tmp;
  int delete_event_index;
  string event_date,
         event_time,
         event_desc;
  string water_goal_string;
  sf::Text confirm_event_text;
  sf::Text delete_event_text;
  Event calendar_event;
  vector<Event> calendar;
  string dashed;
  string water_goal_string2;
  int watergoal = 8; // Default water goal, gets updated based on user preference
...
/////////////////////////////////////////////////////////////
  ...
  Button cal_but;
  vector<Button> calendar_buttons;
  string desc;
  int button_char_size;
  ...
  for(int i = 0; i < calendar.size(); i++) {
    ...
    if(calendar.at(i).buttony < 19) {
      desc = calendar.at(i).description + " " + calendar.at(i).time;
      button_char_size = 8;
      cal_but.setSize({calendar.at(i).buttonx, calendar.at(i).buttony + 4});
    } else {
      desc = calendar.at(i).description +  "\n" + calendar.at(i).time;
      button_char_size = 10;
      cal_but.setSize({calendar.at(i).buttonx, calendar.at(i).buttony});
    }

    cal_but.setString(desc);
    cal_but.setTextColor(sf::Color::Black);
    cal_but.setCharacterSize(button_char_size);
    cal_but.setOutlineColor(sf::Color::Black);
    cal_but.setOutlineThickness(1);
    cal_but.setFont(font);
    cal_but.setTextFill(sf::Color::Black);
    cal_but.setTexture(event_texture);
    calendar_buttons.push_back(cal_but);
  }
...
// Delete event button
  Button delete_event("", {400, 250}, 0, sf::Color::Black);
  delete_event.setFont(font);
  delete_event.centerScreen(window);
...
// Text of the event the user entered
  ...
  // Text of the event the user is attempting to delete
  delete_event_text.setFillColor(sf::Color::Red);
  delete_event_text.setFont(font);
  delete_event_text.setFillColor(sf::Color::Black);
  delete_event_text.setOutlineColor(sf::Color::White);
  delete_event_text.setOutlineThickness(1);
...
// Water popup
  ...
  // Button to close the error messages
...
// Button to close the event popup
  ...
  close_event_pop.setPosition({850, 180});
    ...
    if(increase_by_week) {
      ...
      for(int i = 0; i < calendar.size(); i++) {
        if(check_in_period(local_time, calendar.at(i).d)) {
          ...
          calendar.at(i).in_period = false;
        calendar_buttons.at(i).setFont(font);
        calendar_buttons.at(i).setTextFill(sf::Color::Black);
      }
    }
...
// Draw input for event to window
    ...
    if(add_event_bool) { // Takes users input and creates a corresponding event, error checks for valid date and time
      ...
      if(check_in_period(local_time, calendar.back().d))
        calendar.back().in_period = true;
      else
        calendar.back().in_period = false;

      if(calendar.back().buttony < 19) {
        desc = calendar.back().description + " " + calendar.back().time;
        button_char_size = 8;
        cal_but.setSize({calendar.back().buttonx, calendar.back().buttony + 4});
      } else {
        desc = calendar.back().description + "\n" + calendar.back().time;
        button_char_size = 10;
        cal_but.setSize({calendar.back().buttonx, calendar.back().buttony});
      }
...
// Draw the calendar
    if(calendar.size() > 0) {
      for(int i = 0; i < calendar.size(); i++) {
        if(calendar.at(i).in_period == true) {
          if(check_in_period_by_day(local_time, calendar.at(i).d, 0))
            calendar_buttons.at(i).setPosition({rec_x, 85 + calendar.at(i).button_spacing});
          else if(check_in_period_by_day(local_time, calendar.at(i).d, 1))
            calendar_buttons.at(i).setPosition({2 * (rec_x) + rec_width, 85 + calendar.at(i).button_spacing});
          else if(check_in_period_by_day(local_time, calendar.at(i).d, 2))
            calendar_buttons.at(i).setPosition({3 * (rec_x) + 2 * rec_width, 85 + calendar.at(i).button_spacing});
          else if(check_in_period_by_day(local_time, calendar.at(i).d, 3))
            calendar_buttons.at(i).setPosition({4 * (rec_x) + 3 * rec_width, 85 + calendar.at(i).button_spacing});
          else if(check_in_period_by_day(local_time, calendar.at(i).d, 4))
            calendar_buttons.at(i).setPosition({5 * (rec_x) + 4 * rec_width, 85 + calendar.at(i).button_spacing});
          else if(check_in_period_by_day(local_time, calendar.at(i).d, 5))
            calendar_buttons.at(i).setPosition({6 * (rec_x) + 5 * rec_width, 85 + calendar.at(i).button_spacing});
          else
            calendar_buttons.at(i).setPosition({7 * (rec_x) + 6 * rec_width, 85 + calendar.at(i).button_spacing});

          calendar_buttons.at(i).drawTo(window);
        }

      }
    }

    // Draw forward week/back week buttons
    ...
    // Displaying an error pop up when an error occurs
    if (error_bool) {
    ...
    //Displaying the event count reminder
    if (show_event_count) {
      if(ncount == 1)
        event_popup_message.setString("You have 1 event today!\nRemember to take breaks!");
      else
    ...
    if(delete_event_bool) {
      delete_event.drawTo(window);
      yes.drawTo(window);
      no.drawTo(window);
      sf::FloatRect delete_bounds = delete_event.getGlobalBounds();
      sf::FloatRect delete_text_bounds = delete_event_text.getGlobalBounds();

      delete_event_text.setPosition(
        delete_bounds.left + (delete_bounds.width / 2) - (delete_text_bounds.width / 2),
        delete_bounds.top + (delete_bounds.height / 2) - delete_text_bounds.height);
      window.draw(delete_event_text);
...
/////////////////////////////////////////////////////////////////////
      ...
      if (water_goal_enter) {
        if(event.type == sf::Event::TextEntered) {
          if (event.text.unicode > 47 & event.text.unicode < 58) {
            if(water_goal_string2.length() < 9) {
              tmp = static_cast<char>(event.text.unicode);
